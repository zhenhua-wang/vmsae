...
#' Run vgmsfh using stan.
#'
#' This function runs vgmsfh using stan as the backend.
#'
...
vgmsfh_stan <- function(y, y_sigma, X, W, GEOID,
  ...
  stan_samples <- stan_fit$draws(format = "df")
  vgmsfh <- new("VGMSFH",
    y_hat = as.matrix(stan_samples[, grepl("y_hat", names(stan_samples))]),
    mu = as.matrix(stan_samples[, grepl("mu", names(stan_samples))]),
    beta = as.matrix(stan_samples[, grepl("beta", names(stan_samples))]),
    delta = as.matrix(stan_samples[, grepl("delta", names(stan_samples))]),
    delta_sigma2 = as.matrix(stan_samples[, grepl("delta_sigma2", names(stan_samples))]),
    car = as.matrix(stan_samples[, grepl("phi", names(stan_samples))]))
  return(vgmsfh)
}
